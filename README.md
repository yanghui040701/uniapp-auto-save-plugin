...
可在 HBuilderX 的插件配置中搜索“编辑防抖自动保存”并修改。等价的默认配置示例可用于核对配置文件：
...
点击“不再提示”后，插件才会停止自动提醒；直接关闭弹窗不会持久化抑制状态；下次检查发现原生设置仍关闭时，插件会再次提示，也不影响输入后保存。你也可以随时运行“编辑防抖自动保存：检查并开启 HBuilderX 原生失焦保存”重新检查。

提示状态采用 schema v2：只有用户明确点击“不再提示”后，才会将 `focusSavePromptSuppressed` 持久化为抑制状态；`schemaVersion` 标识该格式。卸载并重新安装插件不会清除这项用户已明确作出的选择；如需重置，请按下文删除状态文件。
...
| 命令面板标题 | 命令 ID | 作用 |
| --- | --- | --- |
| 编辑防抖自动保存：检查并开启 HBuilderX 原生失焦保存 | `yanghui-auto-save.checkFocusSave` | 随时重新检查；未开启时再次询问，已开启时显示确认信息 |
...
使用命令“编辑防抖自动保存：检查并开启 HBuilderX 原生失焦保存”重试；仍失败时，按 `工具 → 设置 → 常用配置 → 失去焦点自动保存` 手动开启。
...
检测到原生设置已经开启时不会提示；原生设置关闭时，插件会继续询问，除非你已明确选择“不再提示”。直接关闭对话框后，下次检查原生设置仍关闭时会再次提示。通常直接运行上面的命令即可再次检查。若要重置“不再提示”选择，参见下一节。
