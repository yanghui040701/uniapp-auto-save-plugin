...
| 短描述 | 停止输入后防抖保存，并协同原生失焦保存 |
| 详细介绍 | 通用 HBuilderX 自动保存插件，不限于 uni-app 项目。停止输入达到配置延迟后，它会重新确认当前活动文件仍是原修改文件，再调用 HBuilderX 当前文件保存命令。原生失焦保存用于补充快速切换文件或应用的场景。 |
| 使用说明 | 安装并重启 HBuilderX 后即可使用；默认启用、延迟 1000 ms。首次激活可授权开启 HBuilderX 原生失焦保存，也可运行命令“自动保存：检查并开启 HBuilderX 原生失焦保存”重新检查。 |
| 更新日志 | 1.0.0（2026-08-06）：加入防抖保存、可配置延迟、过期文档保护、原生失焦保存授权、静默成功与错误提示，并明确隐私声明。 |
| 价格 | 免费（平台产品类型规则） |
| 开源协议 | MIT |
...
| 最低 HBuilderX | 3.2.3+ |
| 已验证环境 | Windows，HBuilderX 5.15.2026070915 |
...
| 审核备注 | 仅监听 HBuilderX 文本与配置事件，并调用 workbench.action.files.save 保存当前活动文件；不执行全部保存，不直接改写文件，不联网，无第三方运行依赖。一次性提示状态在 appData 不可用时回退用户主目录；目录创建或写入失败时，状态可能无法持久化。 |
...
自动保存（防抖增强）是通用 HBuilderX 编辑器插件，不只适用于 uni-app。它监听当前文本文件的编辑变化；停止输入达到设定延迟后，会重新检查插件状态、活动文档身份、dirty 状态、URI 与只读状态，全部满足才通过 HBuilderX 原生命令保存当前活动文件。
...
默认启用，默认延迟 1000 ms；用户可将延迟设置为 200–10000 ms。正常保存不打扰用户，只有保存失败或内部错误才显示提示。

插件不收集或上传文件内容、文件路径与使用数据，不包含遥测、统计分析或广告，不发起网络请求，也不申请特殊权限。未命名、只读、非本地或已切换的文档会被跳过；强制结束 HBuilderX、系统崩溃或断电仍可能使尚未完成正常保存的修改丢失。
...
4. 首次激活时可授权插件开启 HBuilderX 原生失焦保存。自动更新失败时，前往 `工具 → 设置 → 常用配置 → 失去焦点自动保存` 手动开启。
5. 需要重新检查时，从命令面板执行“自动保存：检查并开启 HBuilderX 原生失焦保存”。
...
2. 首次授权对话框：展示开启与暂不开启两个选择；使用空白测试项目拍摄。
...
- 兼容性：最低 HBuilderX 3.2.3；Windows HBuilderX 5.15.2026070915 已验证。不得由这一次 Windows 验证推断 macOS/Linux 已验证。
