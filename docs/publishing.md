...
默认启用，默认延迟 1000 ms；用户可将延迟设置为 200–10000 ms。正常保存不打扰用户，只有保存失败或内部错误才显示提示。启动时原生失焦保存已开启则保持静默；只有在用户点击“开启”后，插件才会修改 `editor.saveOnFocusLost`。只有明确点击“不再提示”才会停止自动提醒；直接关闭弹窗后，下次检查发现原生设置仍关闭时会再次提示。
