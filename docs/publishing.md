...
| 使用说明 | 安装并重启 HBuilderX 后即可使用；默认启用、延迟 1000 ms。启动时已开启的原生失焦保存保持静默；关闭时会询问是否开启。只有点击“开启”才会修改 HBuilderX 设置；只有明确点击“不再提示”才会停止自动提醒，关闭弹窗后下次检查发现原生设置仍关闭时会再次提示。也可运行命令“编辑防抖自动保存：检查并开启 HBuilderX 原生失焦保存”重新检查。 |
| 更新日志 | 1.0.1（2026-08-09）：<br>- 插件显示名称调整为“编辑防抖自动保存”。<br>- 原生失焦自动保存开启时保持静默，之后若被关闭会再次提醒。<br>- 新增明确的“不再提示”选择；关闭弹窗不再永久抑制提醒。<br>- 旧版 `focusSavePromptHandled` 状态不再被视为永久拒绝。 |
| 价格 | 免费（平台产品类型规则） |
| 开源协议 | MIT |
...
编辑防抖自动保存是通用 HBuilderX 编辑器插件，不只适用于 uni-app。防抖保存负责停止输入后的保存；HBuilderX 原生失焦保存覆盖倒计时结束前切换文件或应用的场景。它监听当前文本文件的编辑变化；停止输入达到设定延迟后，会重新检查插件状态、活动文档身份、dirty 状态、URI，并异步检查本地路径当时是否可写，全部满足才通过 HBuilderX 原生命令保存当前活动文件。HBuilderX 明确标记为只读，或本地权限检查返回 `EPERM` / `EACCES` 时会跳过保存。
...
默认启用，默认延迟 1000 ms；用户可将延迟设置为 200–10000 ms。正常保存不打扰用户，只有保存失败或内部错误才显示提示。启动时原生失焦保存已开启则保持静默；只有在用户点击“开启”后，插件才会修改 `editor.saveOnFocusLost`。只有明确点击“不再提示”才会停止自动提醒；直接关闭弹窗不会持久化抑制状态；下次检查发现原生设置仍关闭时会再次提示。

提示状态采用 schema v2：只有用户明确点击“不再提示”后，才会将 `focusSavePromptSuppressed` 持久化为抑制状态；`schemaVersion` 标识该格式。卸载并重新安装插件不会清除这项选择；删除状态文件才会重置它。
...
5. 只有明确点击“不再提示”才会停止自动提醒；直接关闭弹窗后，下次检查发现原生设置仍关闭时会再次提示。需要重新检查时，从命令面板执行“编辑防抖自动保存：检查并开启 HBuilderX 原生失焦保存”。
