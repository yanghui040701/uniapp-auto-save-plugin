...
| 使用说明 | 安装并重启 HBuilderX 后即可使用；默认启用、延迟 1000 ms。启动时已开启的原生失焦保存保持静默；关闭时会询问是否开启。只有点击“开启”才会修改 HBuilderX 设置；只有明确点击“不再提示”才会停止自动提醒，关闭弹窗后下次检查发现原生设置仍关闭时会再次提示。也可运行命令“编辑时自动保存：检查并开启 HBuilderX 原生失焦保存”重新检查。 |
...
5. 只有明确点击“不再提示”才会停止自动提醒；直接关闭弹窗后，下次检查发现原生设置仍关闭时会再次提示。需要重新检查时，从命令面板执行“编辑时自动保存：检查并开启 HBuilderX 原生失焦保存”。
