...
- 最终上传前重新运行项目测试、严格校验和打包命令，并记录产物哈希。
...
7. 选择免费/开源等实际发布方式，阅读页面上的最新协议、隐私和审核提示；不要依据本文推断门户当前的商业选项。
...
- DCloud 官方插件制作和发布指南：[制作和发布插件指南](https://uniapp.dcloud.net.cn/plugin/publish.html)。用于核对插件市场分类背景、包内排除版本控制内容，以及 HBuilderX 插件属于编辑器插件而非手机 App 插件。本项目要求的发布前本地验证属于自己的质量门禁，不把 `uni_modules` 的发布步骤直接套用为 HBuilderX 插件门户事实。
...
登录后的发布门户字段、身份验证要求、图片尺寸、审核策略和操作入口可能随时间变化，也可能因账号或插件类型不同而变化。提交时必须以门户当时显示的必填项、规则与协议为准，做最终复核；本文不把未登录可见资料之外的不确定信息写成既定事实。
