...
test('defaults to not suppressed and persists schema v2 suppression', () => {
  const state = createPromptState({ fs, stateFile });
  assert.equal(state.isSuppressed(), false);
...
test('does not suppress malformed, incomplete, or false state', () => {
  const states = [
    '{bad json',
    JSON.stringify({ schemaVersion: 1, focusSavePromptSuppressed: true }),
    JSON.stringify({ schemaVersion: 3, focusSavePromptSuppressed: true }),
    JSON.stringify({ schemaVersion: 2, focusSavePromptSuppressed: false }),
    JSON.stringify({ schemaVersion: 2 }),
    JSON.stringify({ focusSavePromptSuppressed: true })
  ];

  for (const contents of states) {
    fs.writeFileSync(stateFile, contents, 'utf8');
    assert.equal(createPromptState({ fs, stateFile }).isSuppressed(), false, contents);
  }
});
...
test('treats corrupt state as not suppressed and repairs it', () => {
  fs.writeFileSync(stateFile, '{bad json', 'utf8');
  const state = createPromptState({ fs, stateFile });
  assert.equal(state.isSuppressed(), false);
  assert.doesNotThrow(() => state.suppress());
...
test('treats an unwritable state file as a non-fatal fallback', () => {
  ...
  assert.equal(state.isSuppressed(), false);
  assert.equal(state.suppress(), false);
});

test('publishes suppressed state through unique same-directory temporary files', () => {
  const writes = [];
  const renames = [];
  const fakeFs = {
  ...
  assert.equal(writes[0].contents, '{"schemaVersion":2,"focusSavePromptSuppressed":true}');
...
test('waits for a transient Windows target lock before publishing', {
  ...
  const suppressed = state.suppress();
  assert.equal(await exit, 0, stderr);
  assert.equal(suppressed, true);
  assert.equal(state.isSuppressed(), true);
});

test('concurrent readers never observe partial suppressed-state writes', { timeout: 30000 }, async () => {
  const state = createPromptState({ fs, stateFile });
  assert.equal(state.suppress(), true);
  ...
  const workerSource = `
    ...
    for (let index = 0; index < workerData.writeCount; index += 1) {
      Atomics.add(slots, state.suppress() ? 2 : 3, 1);
    }
  ...
  try {
    ...
    while (Atomics.load(slots, 1) <= workerCount && Date.now() < writeDeadline) {
      for (let index = 0; index < 100; index += 1) {
        samples += 1;
        if (!state.isSuppressed()) falseReads += 1;
      }
