...
function writableFilePath(document) {
...
async function isFilesystemReadOnly(filePath, filesystem) {
  try {
    const promises = filesystem && filesystem.promises;
    const access = promises && promises.access;
    const writableMode = filesystem && filesystem.constants
      ? filesystem.constants.W_OK
      : undefined;
    if (typeof access !== 'function' || typeof writableMode !== 'number') return false;
    await access.call(promises, filePath, writableMode);
    return false;
  } catch (error) {
...
function createHBuilderXRuntime(hx, filesystem = fs) {
  ...
  return {
    ...
    async getActiveSnapshot() {
      ...
      if (!editor || !editor.document) return { document: null, readOnly: false };
      const editorReadOnly = editor.readonly === true || editor.isReadonly === true;
      const filePath = writableFilePath(editor.document);
      if (editorReadOnly || !filePath) {
        return { document: editor.document, readOnly: editorReadOnly };
      }
      const filesystemReadOnly = await isFilesystemReadOnly(filePath, filesystem);
      const currentEditor = await hx.window.getActiveTextEditor();
      if (!currentEditor || !currentEditor.document) {
        return { document: null, readOnly: false };
      }
      const currentEditorReadOnly = currentEditor.readonly === true ||
        currentEditor.isReadonly === true;
      return {
        document: currentEditor.document,
        readOnly: currentEditorReadOnly || (
          writableFilePath(currentEditor.document) === filePath && filesystemReadOnly
        )
      };
    },
